...
class ExtractedDataManager:
                ...
                if path.suffix in docx_endings:
                    ...
                    for page_num in range(len(pdf_doc)):
                        ...
                        if page_text.strip():
                            ...
                            self.logger.warning(
                                f"Página {page_num + 1} de {os.path.basename(path)} não contém texto. Tratando como imagem."
                            )
                            pix = page.get_pixmap()
                            images.append(pix.tobytes())
                    if has_text_content:
                        self.logger.info(
                            f"PDF '{os.path.basename(path)} processado como documento de texto"
                        )
                    pdf_doc.close()
    ...
    def run_extraction(self, file_paths: List[Path], category: str) -> Optional[Dict]:
        self.logger.info(
            f"Iniciando processo de extração para a categoria '{category}'."
        )

        if not file_paths:
            self.logger.warning(
                f"Nenhum arquivo encontrado para a categoria '{category}'. Abortando extração."
            )
            return None
        ...
        if extracted_content["type"] == "text":
            self.logger.info(
                "Conteúdo de texto detectado. Usando o fluxo de extração textual."
            )
            ...
            extracted_json = self.gemini_client.generate_json_from_prompt(
                prompt, model_name
            )

        elif extracted_content["type"] == "images":
            self.logger.info(
                "Conteúdo de imagem detectado. O fluxo de extração multimodal será usado."
            )
            ...
            extracted_json = self.gemini_client.generate_json_from_multimodal_prompt(
                text_prompt=prompt, images=images, model_name=model_name
            )

        elif extracted_content["type"] == "empty":
            self.logger.error(
                "Nenhum conteúdo (texto ou imagem) pôde ser extraído dos arquivos."
            )
            return None

        if not extracted_json:
            self.logger.error(
                "Falha ao extrair dados. A resposta da IA foi nula ou inválida."
            )
            return None
    ...
    def _consolidate_content_fields(self, content_fields: Dict[str, str]) -> str:
        ...
        useful_texts = [
            value.strip() for value in content_fields.values() if value.strip()
        ]
        return "\n\\n".join(useful_texts)
        ...
        for path in file_paths:
            try:
                with fitz.open(path) as doc:
                    self.logger.info(
                        f"Lendo texto do arquivo: {os.path.basename(path)}"
                    )
                    for page in doc:
                        consolidated_text.append(page.get_text())
            except Exception as e:
                self.logger.error(
                    f"Não foi possível ler o arquivo PDF '{path}'. Erro: {e}"
                )
    ...
    def save_dict_to_file(self, project_name: str, category: str, data: Dict) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Dados ds categoria '{category}' para projeto '{project_name}' escritos."
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Erro ao escrever dados de categoria '{category}' em '{file_obj}' para projeto '{project_name}': \n\n {e}"
            )
            return False
