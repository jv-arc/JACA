...
st.set_page_config(
    page_title="JACA - Assistente de Outorga", page_icon="📡", layout="centered"
)
...
# -----------------------------------------------------------------------
def display_files_for_category(
    category_key: str, files: List[str], project_name: str
) -> None:
    for i, file_path in enumerate(files):
        ...
        with col2:
            if st.button(
                "Ver", key=f"view_{category_key}_{i}", use_container_width=True
            ):
        ...
        with col3:
            if st.button(
                "Remover",
                key=f"remove_{category_key}_{i}",
                type="secondary",
                ...
                try:
                    ...
                    logger.info(
                        f"Arquivo '{file_name}' do projeto '{project_name}' com caminho '{file_path}' removido da categoria '{category}'"
                    )
                    ...
                    logger.error(
                        f"Erro ao remover arquivo '{file_name}' do projeto '{project_name}' com caminho '{file_path}' removido da categoria '{category}' \n\n {e}"
                    )
...
def render_category_uploader(category_config: Dict[str, Any], project_data: Any):
    ...
    if files:
        ...
        logger.info(
            f"Categoria '{category_key}' para projeto '{project_name}' não possui nenhum arquivo"
        )
        ...
        if st.button(
            ...
            for uploaded_file in files_to_upload:
                if uploaded_file:  # Garante que o arquivo não é None
                    if project_manager.add_file(
                        project_name, uploaded_file, category_key
                    ):
                        logger.info(
                            f"Arquivo '{uploaded_file}' adicionado ao projeto '{project_name}' na categoria '{category_key}' com sucesso"
                        )
                        success_count += 1
                    else:
                        logger.error(
                            f"Erro ao adicionar arquivo '{uploaded_file}' ao projeto '{project_name}' na categoria '{category_key}'"
                        )
...
if st.session_state.current_project:
    ...
    st.markdown(
        "Gerencie aqui os documentos do seu projeto. Faça o upload dos arquivos necessários para cada categoria antes de prosseguir para a extração de dados."
    )
    st.divider()
    ...
    try:
        ...
        logger.error(
            f"Falaha ao tentar carregar dados do projeto '{project_name}' \n\n {e}"
        )
        st.session_state.current_project = None
        st.stop()
    ...
    CATEGORIES = [
        {
            "key": "estatuto",
            "title": "Estatuto da Organização",
            "description": "Faça o upload do arquivo do estatuto social, em PDF ou DOCX.",
            "multiple": False,
        },
        {
            "key": "ata",
            "title": "Ata de Eleição da Última Diretoria",
            "description": "Anexe a ata de eleição e posse da diretoria atual.",
        },
        {
            "key": "identificacao",
            "title": "Documentos de Identificação dos Diretores",
            "description": "Anexe os RGs e CPFs de todos os diretores.",
        },
        {
            "key": "licenca",
            "title": "Licença de Funcionamento (se houver)",
            "description": "Anexe licenças prévias ou existentes.",
        },
        {
            "key": "programacao",
            "title": "Grade de Programação",
            "description": "Anexe o documento com a grade de programação semanal.",
        },
    ]
    ...
    st.info(
        "💡 **Pronto?** Prossiga para a página **Extrair Texto** no menu ao lado para iniciar a análise com IA."
    )
    ...
    if existing_projects:
        with st.form("load_project_form"):
            selected_project = st.selectbox(
                "Ou selecione um projeto existente", existing_projects
            )
            submitted = st.form_submit_button(
                "Carregar Projeto", use_container_width=True
            )
            if submitted:
